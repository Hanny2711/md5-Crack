...
if ( isset($_GET['code']) ) {
    ...
    if ( strlen($code) != 4 ) {
        $error = "Input must be exactly four digits";
                ...
                $code[2] < "0" || $code[2] > "9" ||
                $code[3] < "0" || $code[3] > "9") {
        $error = "Input must be four digits numbers between 0000 and 9999.";
    } else {
        $md5 = hash('md5', $code);
    }
}
?>
<!DOCTYPE html>
<head><title>Hanny Ko Ko PIN Code</title></head>
<body>
<h1>MD5 PIN Maker</h1>
<?php
if ( $error !== false ) {
...
?>
<p>Please enter a four-digit number for encoding.</p>
<form>
